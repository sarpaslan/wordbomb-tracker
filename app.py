...
@app.route("/admin/user/<int:user_id>/delete-bug-data", methods=["DELETE"])
@limiter.limit("5 per minute") # Extra protection against misuse
def delete_user_bug_data(user_id):
    """
    Deletes all bug-related data for a specific user.
    This is a protected endpoint requiring a secret key.
    """
    # 1. --- SECURITY CHECK ---
    # Get the secret key from your .env file
    admin_secret = os.environ.get("ADMIN_SECRET_KEY")
    if not admin_secret:
        return jsonify({"error": "CRITICAL: Admin secret key is not configured on the server."}), 500

    # Get the secret key sent by the client in the request headers
    ...
    try:
        ...
        # Delete from the detailed bug_reports table first
        cursor.execute("DELETE FROM bug_reports WHERE user_id = ?;", (user_id,))
        reports_deleted_count = cursor.rowcount # Get how many rows were deleted

        # Delete from the summary bug_points table
        cursor.execute("DELETE FROM bug_points WHERE user_id = ?;", (user_id,))
        points_deleted_count = cursor.rowcount # Get how many rows were deleted

        # Commit the changes to the database
        conn.commit()

        # Invalidate the cache for the user's bug details page
        cache.delete_memoized(user_bug_points_details, user_id)
        # Invalidate the cache for the main user stats page
        cache.delete_memoized(api_user, user_id)


        return jsonify({
            "status": "success",
            "message": f"All bug data for user {user_id} has been deleted.",
            "details": {
                "individual_reports_deleted": reports_deleted_count,
                "summary_records_deleted": points_deleted_count
            }
        }), 200
        ...
        if conn:
            ...
            conn.rollback()
        print(f"[ERROR] Failed to delete bug data for user {user_id}: {e}")
